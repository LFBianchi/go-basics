...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		...
		_, okRune := nameCounts[nameRune]
		if !okRune {
			nameCounts[nameRune] = make(map[string]int)
		}
		_, okName := nameCounts[nameRune][name]
		if !okName {
			nameCounts[nameRune][name] = 0
		}
		nameCounts[nameRune][name] += 1
	}
	return nameCounts
}
